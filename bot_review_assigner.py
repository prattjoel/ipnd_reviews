...
GET_PLACE_IN_LINE = '{}/submission_requests/{}/waits'
...
def check_place_in_line (current_request):
    logger.info('Checking place in line')
    get_line_place = requests.get(GET_PLACE_IN_LINE.format(BASE_URL, current_request['id']), headers=headers)

    get_line_place.raise_for_status()
    if get_line_place.status_code == 401:
        logger.info('Error getting place in line')
        return None
    else:
        return get_line_place.json()
...
def request_reviews(token):
    ...
    while True:
        ...
        if current_request is None:
            ...
            current_place_in_line = check_place_in_line(current_request)
            logger.info(current_place_in_line)
        else:
            ...
            refresh_interval = utcnow + timedelta(minutes=2)

            if closing_at < refresh_interval:
...
if __name__ == "__main__":
	...
	action='store', default=os.environ.get('UDACITY_REVIEWER_TOKEN'),
	help="""
